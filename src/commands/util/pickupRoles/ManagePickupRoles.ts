...
export class ManagePickupRoles extends Command {
    ...
    return void (await interaction.client.prisma.pickupRole
      ...
      .catch(async () => {
        throw await resolveKey(
          interaction,
          "commands/util/pickuprole:manageFailedToRemoveRole"
        );
      })
      .then(async (role) => {
        return void EditReplyStatusEmbed(
          ...
          await resolveKey(
            interaction,
            "commands/util/pickuprole:manageSuccessRemove",
            { role: { id: role.roleID } }
          ),
          interaction
        );
  ...
  private async createRole(interaction: CommandInteraction) {
    ...
    try {
      ...
      throw await resolveKey(
        interaction,
        "commands/util/pickuprole:manageFailedToAddRole"
      );
    } finally {
      return void EditReplyStatusEmbed(
        true,
        await resolveKey(
          interaction,
          "commands/util/pickuprole:manageSuccessAdd",
          { role }
        ),
        interaction
      );
