...
module.exports = {
    ...
    async execute(interaction, client) {
        await interaction.reply("<a:loading:877782934696919040> Fetching Info `(This will hang if " +
            interaction.options.getString("username") +
            " hasn't visited the End and the Nether and died atleast once)`");
        var url = "http://202.131.88.29:25571/player/" +
            interaction.options.getString("username") +
            "/raw";
        try {
            getJSON(url, function (error, response) {
                ...
                if (response.world_times.times.world_the_end.times.SURVIVAL ===
                    "undefined") {
                    throw "DIM";
                }
