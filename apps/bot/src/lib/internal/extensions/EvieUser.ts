...
import { EviePlus, EvieUser as DBEvieUser, UserFlags as EvieUserFlags } from "@prisma/client";
...
export class EvieUser {
	...
	public static async getBadges(user: User, fetchedUser?: FetchedEvieUser): Promise<string> {
		const badges: string[] = [];
		...
		badges.push(...evieUser.flags.map((flag) => this.evieUserFlagToEmoji(flag)));
