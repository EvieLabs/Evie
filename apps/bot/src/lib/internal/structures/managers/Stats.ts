...
export class Stats {
	...
	public static commitName = execSync("git log -1 --pretty=%an").toString().trim();

	public static commitNumber = execSync("git rev-list --all --count").toString().trim();

	public static currentBranch = execSync("git rev-parse --abbrev-ref HEAD").toString().trim();
