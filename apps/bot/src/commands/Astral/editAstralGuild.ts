...
export class EditAstralGuild extends Command {
	public override async messageRun(message: Message, args: Args) {
		...
		const configValue = message.cleanContent.split(" ").slice(3).join(" ") || null;
		const looksLikeJson =
			configValue &&
		...
		if (!configOption) {
			return void (await message.reply({
				...
				files: [
					new MessageAttachment(Buffer.from(JSON.stringify(config, null, 2)), `astral-guild-config-${guildId}.json`),
				],
			}));
		}
		...
		const newConfig = await this.container.client.prisma.astralConfig
			...
			.catch((e) => {
				throw ["Failed to update!", "```js", JSON.stringify(e, null, 2), "```"].join("\n");
			});

		return void (await message.reply({
			content: `Astral Guild Config for ${inlineCode(guildId)}`,
			files: [
				new MessageAttachment(Buffer.from(JSON.stringify(newConfig, null, 2)), `astral-guild-config-${guildId}.json`),
			],
		}));
	}
}
