...
export class Tag extends Command {
	public override async chatInputRun(interaction: CommandInteraction): Promise<void> {
		...
		const tag = (await interaction.client.db.FetchTags(interaction.guild)).find((tag) => tag.id === query);
		if (!tag) {
		...
		if (tag.embed) {
			void interaction.reply({
				...
				}! ${interaction.user.toString()} thinks this will be useful for you!`,
				embeds: [e],
				allowedMentions: {},
			});
		} else {
			void interaction.reply({
				content: `${
					target
						? `Hey, ${target.toString()}! ${interaction.user.toString()} thinks this will be useful for you!\n`
						: ""
				}${tag.content}`,
				allowedMentions: {},
			});
