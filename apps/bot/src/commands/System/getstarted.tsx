...
import { ApplicationCommandRegistry, Command, container, RegisterBehavior } from "@sapphire/framework";
...
export class GetStarted extends Command {
	private async getStarted(ctx: Message | CommandInteraction) {
		...
		const description = [
			...
			await resolveKey(ctx, "commands/getstarted:onlyShowingCommandsYouHavePermsFor"),
		].join("\n\n");
	...
	private async categorizeCommands(
		...
		for (const command of commandStore) {
			if (command.category) {
				...
				const parsedCategory = `${command.category}${command.subCategory ? `/${command.subCategory}` : ""}`;
				...
				const precons =
					context instanceof Message
						? await command.preconditions.messageRun(context, command as never, {
								// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
								command: null!,
						  })
						: await command.preconditions.chatInputRun(context, command as never, {
								// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
								command: null!,
						  });
