...
export class PickupRole extends Command {
	public override async chatInputRun(interaction: CommandInteraction) {
		...
		if (!pickUpRole) throw await resolveKey(interaction, "commands/util/pickuprole:roleNotInDatabase");

		const role = guild.roles.cache.get(pickUpRole.roleID) ?? (await guild.roles.fetch(pickUpRole.roleID));

		if (!role) throw await resolveKey(interaction, "commands/util/pickuprole:roleNotInGuild");

		const removeRole = interaction.member.roles.cache.has(role.id);

		removeRole
			? await interaction.member.roles.remove(role).catch(async () => {
					throw await resolveKey(interaction, "commands/util/pickuprole:failedToRemoveRole", {
						role: role,
					});
			  })
			: await interaction.member.roles.add(role).catch(async () => {
					throw await resolveKey(interaction, "commands/util/pickuprole:failedToAddRole", {
						role: role,
					});
			  });
		...
		void interaction.editReply({
			embeds: [
				new StatusEmbed(
					true,
					await resolveKey(interaction, `commands/util/pickuprole:${removeRole ? "successRemove" : "successAdd"}`, {
						role: role,
					}),
				),
			],
