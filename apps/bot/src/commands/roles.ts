    ...
    if (perms.has(Permissions.FLAGS.MANAGE_ROLES)) {
      ...
      if (subcommand == "joinrole") {
        ...
        if (enable) {
          if (role) {
            try {
              await RolesDB.setJoinRole(interaction.guild!, role);
            } catch (error) {
              interaction.editReply({
                content:
            ...
            await RolesDB.setJoinRoleEnable(interaction.guild!, true);
            const e = await EvieEmbed(interaction.guild!);
            e.setDescription(
          ...
          await RolesDB.setJoinRoleEnable(interaction.guild!, false);
          const e = await EvieEmbed(interaction.guild!);
