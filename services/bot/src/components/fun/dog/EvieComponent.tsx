...
export default function EvieComponent(props: { user: User }) {
	...
	async function randomEvie() {
		const res = await axios.get(`https://raw.githubusercontent.com/twisttaan/AxolotlBotAPI/main/evie.txt`);
	...
	return (
		<>
			<Embed color={EvieColors.evieGrey} description={image ? undefined : "Loading..."} image={{ url: image ?? "" }} />
			<Button
				style="primary"
				user={props.user}
				label="Another One"
