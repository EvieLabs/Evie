...
export default function RoleMenu(props: { member: GuildMember; roles: SelectRole[]; roleDivider?: string }) {
		...
		<>
			...
			<Select
				...
				onChangeValue={async (value, interaction) => {
					...
					member.roles.cache.has(value) ? await member.roles.remove(value) : await member.roles.add(value);
					...
					interaction.ephemeralReply(
						<Embed
							...
							description={roles
								.map((role) =>
									removeIndents(
										`${role.emoji} ${role.name}: ${member.roles.cache.has(role.id) ? Emojis.enabled : Emojis.disabled}`,
									),
								)
								.join("\n")}
						/>,
			...
			>
				{roles.map((role) => (
					<Option key={role.id} value={role.id} label={role.name} description={role.description} emoji={role.emoji} />
				))}
			</Select>
		</>
	);
