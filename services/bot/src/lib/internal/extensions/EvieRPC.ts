...
export class EvieRPC extends Server {
	private findServicePath(service: ServiceDefinition): string {
		...
		return this.findServicePath(service[children[0]] as unknown as ServiceDefinition);
	}

	public constructor(sImpls: [ServiceDefinition, UntypedServiceImplementation][], options?: ChannelOptions) {
		super(options);
		sImpls.forEach(([service, impl]) => {
			this.addService(service, impl);
			console.log(magenta(`[gRPC]`), blue(`Added service ${this.findServicePath(service)}`));
		});
		...
		this.bindAsync(
			...
			(err, port) => {
				...
				console.log(magenta(`[gRPC]`), blue(`Server Listening on 127.0.0.1:${port}`));
				this.start();
			},
		);
	}
