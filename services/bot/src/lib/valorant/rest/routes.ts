...
import type { AccountData, MatchHistoryDataV3, MMRDataV2 } from "../types/types";

export async function fetchCompStats(props: { region: string; name: string; tag: string }) {
	try {
		const res = await fetchData<MMRDataV2>(`/valorant/v2/mmr/${props.region}/${props.name}/${props.tag}`);
...
export async function fetchMatchHistory(props: { region: string; name: string; tag: string; puuid: string }) {
	try {
...
export async function fetchAccountData(props: { name: string; tag: string }) {
	try {
		const res = await fetchData<AccountData>(`/valorant/v1/account/${props.name}/${props.tag}`);
