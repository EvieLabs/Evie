...
import type { OfflineServerResponse, OnlineServerResponse } from "@evie/interfaces";
...
export class MinecraftServer extends Command {
	...
	public override async chatInputRun(interaction: CommandInteraction) {
		...
		if (mcServer) return void (await this.handleMinecraftServer(mcServer, interaction));
	...
	private async handleMinecraftServer(
		...
		switch (mcServer.online) {
			case true: {
				...
				try {
					const { data: image } = await this.container.client.evieRest.post<Buffer>(
				...
				const basicInfo = [
					`Players: ${inlineCode(`${mcServer.players.online}/${mcServer.players.max}`)}`,
				...
				const advancedInfo = [
					...
					`Data from: ${cachetime === 0 ? time(new Date(), "R") : time(new Date(cachetime * 1000), "R")} 
        `,
				].map((line) => `${Emojis.bulletPoint} ${line}`);
	...
	private async pingMinecraftServer(hostname: string): Promise<OnlineServerResponse | OfflineServerResponse | null> {
		try {
			const data = await fetch<OnlineServerResponse | OfflineServerResponse>(
				`https://api.mcsrvstat.us/2/${hostname}`,
				FetchResultTypes.JSON,
			);
			return data.online ? data : data;
		} catch (e) {
			return null;
		}
	}
		...
		registry.registerChatInputCommand(
			{
				...
				options: [
					{
						name: "destination",
						description: "The hostname or IP of the server to ping. (e.g. hypixel.net)",
						type: "STRING",
						required: true,
					},
					{
