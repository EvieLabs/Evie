...
import { capitalizeEachWord, trimArray } from "#root/utils/builders/stringBuilder";
...
import { ContextMenuInteraction, GuildMember, Message, SnowflakeUtil, User } from "discord.js";
...
export class UserInfo extends Command {
	public override async messageRun(message: Message, args: Args) {
		const member = await args.pick("member").catch(async () => {
			...
			return message.client.users.fetch(snowflake).catch(() => null);
		});
	...
	public override async contextMenuRun(interaction: ContextMenuInteraction) {
		if (!interaction.inCachedGuild()) throw await resolveKey(interaction, "errors:notInCachedGuild");
	...
	private async sendUserInfo(interaction: ContextMenuInteraction<"cached"> | Message, user: GuildMember | User) {
		const badges = await EvieUser.getBadges(user instanceof User ? user : user.user).catch(() => "");

		const description: string[] = [];

		if (user instanceof GuildMember) {
			description.push(`Known as: ${user.displayName}`);
			description.push(`Joined ${user.guild.name}: ${user.joinedAt ? time(user.joinedAt, "R") : "Unknown"}`);
		}

		const createdAt = SnowflakeUtil.deconstruct(user.id).date;

		description.push(`Created Account: ${time(createdAt, "R")}`);

		const tag = user instanceof User ? user.tag : user.user.tag;

		const embed = new EvieEmbed().setThumbnail(user.displayAvatarURL()).setTitle(`${tag} (${user.id})`).setDescription(`
        ${badges}
		...
		user instanceof GuildMember &&
			embed.addFields([
				{
					name: "Roles",
					value: trimArray(user.roles.cache.map((r) => `${r.toString()}`)).join(", "),
				},
				{
					name: "Permissions",
					value: capitalizeEachWord(trimArray(user.permissions.toArray()).join(", ").replace(/\_/g, " ").toLowerCase()),
				},
			]);
