...
import { CommandInteraction, Invite, Message, PartialGroupDMChannel } from "discord.js";
...
export class InviteInfo extends Command {
	public override async messageRun(message: Message, args: Args) {
		...
		const inviteInfo = await message.client.fetchInvite(invite).catch(() => null);
	...
	public override async chatInputRun(interaction: CommandInteraction) {
		...
		const inviteInfo = await interaction.client.fetchInvite(invite).catch(() => null);
		...
		return void (await interaction.editReply(await this.getInviteInfo(inviteInfo)));
	}

	private getInviteInfo(invite: Invite) {
		const { guild, channel } = invite;

		if (!guild || channel instanceof PartialGroupDMChannel) throw "Failed to fetch invite info.";

		const inviteInfo: string[] = [];

		inviteInfo.push(`Created invite: ${invite.createdAt ? time(invite.createdAt, "R") : "Unknown"}`);

		inviteInfo.push(`Channel name: ${channel.name}`);

		inviteInfo.push(`Channel type: ${inlineCode(channel.type)}`);

		inviteInfo.push(`Expires: ${invite.expiresAt ? time(invite.expiresAt, "R") : "Never"}`);

		inviteInfo.push(`Max uses: ${invite.maxUses ? invite.maxUses : "Unlimited"}`);

		inviteInfo.push(`Inviter: ${invite.inviter ? `${invite.inviter.tag} (${invite.inviter.id})` : "Unknown"}`);
		...
		guildInfo.push(`Verification level: ${inlineCode(guild.verificationLevel)}`);

		const embed = new EvieEmbed().setTitle(guild.name).setFields([
			{
				name: "Invite Info",
				value: inviteInfo.map((line) => `${Emojis.bulletPoint} ${line}`).join("\n"),
			},
			{
				name: "Server Info",
				value: guildInfo.map((line) => `${Emojis.bulletPoint} ${line}`).join("\n"),
			},
		]);
		...
		registry.registerChatInputCommand(
			{
				...
				options: [
					{
						name: "invite",
						description: "The invite to fetch information for. (e.g. discord.gg/discord-townhall, discord-townhall)",
						type: "STRING",
						required: true,
					},
					{
