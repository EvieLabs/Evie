...
import { constructMessageLink, removeIndents } from "#root/utils/builders/stringBuilder";
...
export class Cases extends Command {
	public override async chatInputRun(interaction: CommandInteraction) {
		...
		try {
			const modLog = await interaction.client.guildLogger.getModChannel(interaction.guild);
			...
			).sort((a, b) => SnowflakeUtil.deconstruct(b.id).date.getTime() - SnowflakeUtil.deconstruct(a.id).date.getTime());

			const casesFormatted = cases.map((case_) => {
				return {
					description: removeIndents(
						`**Case**: [#${case_.id}](${constructMessageLink(modLog, case_.logMessageID ?? case_.id)}) (${
							case_.type || `\`${case_.typeId}\``
						})
      **Reason**: ${case_.reason}
      ${`**Moderator**: ${case_.moderatorName ?? "Automated"} (\`${case_.moderatorID ?? "Automated"}\`)`}
