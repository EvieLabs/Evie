...
export class ContextMenuCommandErrorListener extends Listener {
	public async run(error: Error, { interaction }: ContextMenuCommandErrorPayload) {
		...
		return interaction.deferred
			? void EditReplyStatusEmbed(false, await resolveKey(interaction, "errors:commandError"), interaction)
			: void ReplyStatusEmbed(false, await resolveKey(interaction, "errors:commandError"), interaction);
	}
}
