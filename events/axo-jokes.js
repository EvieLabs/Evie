...
module.exports = {
    ...
    async execute(message) {
        ...
        if(msg === "!apply" || msg === "!smp"){
            ...
            exampleEmbed.addField("How do I apply?", "Once you have the channels un hidden you can apply in <#877878230722437180> for <@&878074525223378974> so you can actually build/break instead of just exploring!")
            message.channel.send({ embeds: [exampleEmbed] })
            // message.delete()
        }
        ...
        if(msg.includes("531656855433379848") || msg.includes("794485891740729344") || msg.includes("491566122546495488") || msg.includes("97470053615673344")){
            let auth = message.author.toString()
            if(auth.includes(`531656855433379848`) || auth.includes(`794485891740729344`) || auth.includes(`491566122546495488`) || auth.includes("97470053615673344")){
                return
            }
